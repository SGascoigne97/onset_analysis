...
%         tab_fill, tab_fill, 'VariableNames', ["Patient_id", "Outcome", "resected_count", resected_prop]);
% else
%     resec_size_tab = table(final_output.Patient_id, final_output.outcome_cat,...
%         tab_fill, tab_fill, tab_fill, 'VariableNames', ["Patient_id", "Outcome", ...
%         repelem("resected",3)+ ["_count", "_prop", "_vol"]]);
% end
% 
% resec_size_tab.impl_size =...
...
%         resec_size_tab(subj,:).resected_prop = sum(resec)/resec_size_tab(subj,:).impl_size;
%     end
% 
%     if contains(chan_or_roi, "roi")
%         regions = subj_onset.(sprintf("roi_names_%s", extractAfter(chan_or_roi, "roi_"))){:};
...
% if chan_or_roi == "chan"
%     comp_vars = [comp_vars(contains(comp_vars, "count")), comp_vars(contains(comp_vars, "prop"))];
%     half_violin(resec_size_tab, comp_vars(1),...
%         double(resec_size_tab.Outcome == out_grps(2)), ...
%         "bin_wds", 1,"save_fig",save_fig, "y_lim", [0,14],...
%         "file_type","svg", "save_loc", sprintf("%scount_violins", save_loc), ...
%         "grp_names", out_grps, "legend_pos", "northeast")
%     half_violin(resec_size_tab, comp_vars(2),...
%         double(resec_size_tab.Outcome == out_grps(2)), ...
%         "bin_wds", 0.1,"save_fig",save_fig, "y_lim", [0,1],...
%         "file_type","svg", "save_loc", sprintf("%sprop_violins", save_loc), ...
%         "grp_names", out_grps, "legend_pos", "northeast")
% else
%     comp_vars = [comp_vars(contains(comp_vars, "count")),...
%         comp_vars(contains(comp_vars, "prop")),...
%         comp_vars(contains(comp_vars, "vol"))];
%      half_violin(resec_size_tab, comp_vars,...
%         double(resec_size_tab.Outcome == out_grps(2)), ...
%         "bin_wds", [1,0.1,NaN],"save_fig",save_fig,...
%         "y_lim", [0,14; 0,1; 0, 3*10^5],...

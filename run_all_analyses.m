...
f = figure();
f.Position = [10,10,1000,1000];
cm = [repmat([1,1,1],95,1); repmat([1,0,0],5,1)];
tiledlayout(5,1)
for comp = comps
    nexttile
...
%% Repeating for unimodal cases
%% Repeat analysis using subjects with confirmed unifocal epilepsy
unifocal = "UCLH" + string([592,687,718,756,770,800,804,821,851,852,863,...
    895,931,934,936,940,943,965,985,998,999,1006,1025,1030,1038,1045,1075,...
    1085,1091,1106,1109,1110,1113,1116,1119,1135,1149,1163,1168,1179,1182,...
    1196,1200,1211,1220,1236,1379,1412]);

load("tables/final_output_not_anon.mat")
load('roi_info/ATLAS.mat')
final_output = final_output_not_anon(final_output_not_anon.outcome ~= 8,:);

% final_output.Properties.VariableNames(1:2) = {"Patient_id", "orig_id"}
% Pull out the unifocal cases form final_output
final_output_unifocal = final_output(contains(final_output.Patient_id,unifocal),:);

% We will save final_output as final_output_orig and rename
% final_output_unifocal as final_output to run the analysis again
final_output_orig = final_output;
final_output = final_output_unifocal;
...
%                                           % be created to store figures and tables
save_fig = 1; % Each figure that is created will be saved 
save_folder = 'figures/paper_figures/new_results/unifocal/'; % Location where subfolders will
                                          % be created to store figures and tables
...
%% Repeat comparison of parecellations with this subgroup
clear combined_tab
for chan_or_roi = ["chan", "roi_120", "roi_250"]
    for comp = ["clo_concord", "onset_resec"]
        fields = string(fieldnames(auc_tabs.(sprintf(chan_or_roi)).(sprintf(comp))));
...
%                     subplot(121)
                    ...
                    title(sprintf("Spearman's rank \n r(%d) = %.3f, p = %.3f", length(grp_1_vals)-2, r, p))
                    saveas(f, sprintf('%s/onset_size/%s_%s(%s_v_%s).svg', save_folder, det, comp, parcs(parc1), parcs(parc2)))
